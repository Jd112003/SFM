...
for obj in estatua leon; do
    echo "============================================="
    echo "Processing ${obj} from scratch (Dense Reconstruction n80)..."
    echo "============================================="
    ...
    # Clean previous failed attempts completely
    docker compose run -T --user root --rm --entrypoint rm sfm -rf "/workspace/${dense_dir}"
    mkdir -p "${dense_dir}"
...
mesh.export('/workspace/outputs/${obj}/reconstruction/model_dense_n80.stl')
    ...
    docker compose run -T --rm --user root --entrypoint chown sfm \
        -R ${SFM_UID}:${SFM_GID} /workspace/${dense_dir} /workspace/outputs/${obj}/reconstruction/model_dense_n80.stl || true
